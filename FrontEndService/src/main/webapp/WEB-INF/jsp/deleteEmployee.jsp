...
<body>
    ...
    <h1 class="bg-indigo-600 text-white py-2 px-6 rounded-full text-xl mt-6 " style="width: 21%;
    margin-left: 40%;margin-top: 0%;">Delete Employee</h1>
    </div>
    </div>
    
  

    <form class="ml-4" action="deleteEmployee" method="post" style="margin-left:40%;">
        <p>Enter ID: </p>
        <input type="number" name="employeeId" class="border border-solid border-gray-400 p-2 rounded-md">
        <input type="submit" value="Submit" class="bg-indigo-600 text-white py-2 px-4 rounded-full text-xl mt-2">
    </form>
      <%
        Map<String, Object> serverResponse = (Map<String, Object>) request.getAttribute("serverResponse");
        if (serverResponse != null){
    %>
        <p class="ml-4" style="margin-left:40%;"><%=serverResponse.get("message") %></p>
    <%} %>
